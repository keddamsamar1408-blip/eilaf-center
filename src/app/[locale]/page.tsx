...
export default async function HomePage() {
  ...
  const tagline = locale === "fr" ? settings.tagline_fr : locale === "en" ? settings.tagline_en : settings.tagline_ar;
  const centerName =
    locale === "fr" ? settings.center_name_fr : locale === "en" ? settings.center_name_en : settings.center_name_ar;

  const featured = listItems({}).filter((i) => i.featured).slice(0, 6);
  const allPublished = listItems({});
  ...
  const services = [
    ...
    { icon: "🌱", title: t("service5Title"), desc: t("service5Desc") },
  ];
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-cream via-cream-light to-white pattern-bg">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24 lg:px-8">
          <div className="animate-fade-up">
            ...
            <p className="mt-5 text-lg leading-relaxed text-ink/70">{tagline || t("heroTagline")}</p>
          ...
          <div className="relative animate-fade-up">
            <div className="relative aspect-square overflow-hidden rounded-3xl shadow-2xl ring-8 ring-white sm:aspect-[4/3]">
              <Image src={settings.cover_url} alt={centerName} fill className="object-cover" priority sizes="(max-width:1024px) 100vw, 50vw" />
            </div>
            <div className="absolute -bottom-6 -start-6 hidden h-28 w-28 items-center justify-center rounded-2xl bg-white p-3 shadow-xl ring-1 ring-black/5 sm:flex">
              <Image src={settings.logo_url} alt="logo" width={90} height={90} className="rounded-xl object-cover" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-10 sm:px-6 md:grid-cols-4 lg:px-8">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <div className="text-3xl font-extrabold text-gold sm:text-4xl">{s.value}</div>
              <div className="mt-1 text-sm text-cream-light/80">{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-ink/60">{t("servicesSubtitle")}</p>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5">
          {services.map((s) => (
            ...
            >
              ...
              <h3 className="mt-4 font-bold text-maroon-dark">{s.title}</h3>
      ...
      <section className="bg-cream py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-ink/60">{t("modesSubtitle")}</p>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
            {[
              { icon: "🏢", title: t("modeInPersonTitle"), desc: t("modeInPersonDesc") },
              { icon: "💻", title: t("modeZoomTitle"), desc: t("modeZoomDesc") },
              { icon: "🎥", title: t("modeMeetTitle"), desc: t("modeMeetDesc") },
            ].map((m) => (
              <div key={m.title} className="card-hover rounded-2xl bg-white p-8 text-center shadow-sm">
                ...
                <h3 className="mt-5 text-lg font-bold text-maroon-dark">{m.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
          <div>
            ...
            <p className="mt-3 text-ink/60">{t("featuredSubtitle")}</p>
          </div>
        </div>
        ...
        {featured.length === 0 && allPublished.length === 0 ? (
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {(featured.length ? featured : allPublished.slice(0, 6)).map((item) => (
      ...
      <section className="relative overflow-hidden bg-gradient-to-r from-maroon to-maroon-dark py-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-cream-light/90">{t("ctaSubtitle")}</p>
          <a
